...
function isPublic(pathname: string) {
  if (pathname.startsWith("/profile") && !pathname.startsWith("/profile/update")) {
    return true;
  }
...
export async function updateSession(request: NextRequest) {
  ...
  // if (pathname.startsWith("/profile/update")) {
  //   return NextResponse.redirect(new URL("/join", request.url));
  // }


  // onboarding guard (loop-safe)
  if (role !== "non-member" && !setupDone && !pathname.startsWith("/profile/update")) {
